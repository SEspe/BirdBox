...
/* Per-setting info popup (v2.65): the small circled-i next to each Settings
 * label opens one shared modal (description / default / alternatives) filled
 * from the SINFO table in the JS — the long always-visible help paragraphs
 * moved in there. */
".inf{display:inline-block;width:15px;height:15px;line-height:14px;text-align:center;"
...
"<h3 class='sh' style='margin-top:0'>Placement<span class='inf' onclick='sInfo(\"mode\")'>i</span></h3>"
...
"<label class='wl'>Cool-down between events (s)<span class='inf' onclick='sInfo(\"cool\")'>i</span></label>"
"<input class='wi' type='number' min='1' max='600' id='stCool'>"
"<label class='wl'>Detection quarantine after boot (s)<span class='inf' onclick='sInfo(\"qtn\")'>i</span></label>"
"<input class='wi' type='number' min='0' max='3600' id='stQtn'>"
"<label class='wl'><input type='checkbox' id='stZoom'> Zoom species ID to the motion area<span class='inf' onclick='sInfo(\"dzoom\")'>i</span></label>"
"<label class='wl'><input type='checkbox' id='stFshut'> Reduce motion blur (fast shutter)<span class='inf' onclick='sInfo(\"fshut\")'>i</span></label>"
"<h3 class='sh'>Species Identification</h3>"
"<label class='wl'>Confidence threshold (%)<span class='inf' onclick='sInfo(\"conf\")'>i</span></label>"
"<input class='wi' type='number' min='15' max='95' id='stConf'>"
"<label class='wl'>Species set<span class='inf' onclick='sInfo(\"rfilt\")'>i</span></label>"
...
"<label class='wl'><input type='checkbox' id='stInatCv'> Identify new birds with iNaturalist first<span class='inf' onclick='sInfo(\"inatcv\")'>i</span></label>"
"<label class='wl'>iNaturalist username<span class='inf' onclick='sInfo(\"iuser\")'>i</span></label>"
...
"<p class='sts' style='margin-top:2px'><b>Security:</b> the password is stored on the device in plain "
"text (a flash dump would reveal it) and is never shown back or exported.</p>"
"<label class='wl'>Location (nearest capital)<span class='inf' onclick='sInfo(\"loc\")'>i</span></label>"
...
"<p class='sts' style='margin-top:10px'><b>Advanced &mdash; manual fallback.</b> Only needed if the "
"automatic login stops working.</p>"
"<label class='wl'>Session cookie (mints tokens for weeks)<span class='inf' onclick='sInfo(\"isess\")'>i</span></label>"
"<input class='wi' type='password' id='stIsess' autocomplete='off' placeholder='(not set)'>"
"<label class='wl'>iNaturalist API token (JWT, ~24 h)<span class='inf' onclick='sInfo(\"ikey\")'>i</span></label>"
...
"<p class='sts' style='margin-top:2px'>Escalates the events iNaturalist could not identify to a paid "
"cloud vision model. <b>Costs money</b> against your own API key, and <b>photos leave the device</b> "
"while a provider is selected &mdash; leave Off unless you are actively gathering labels.</p>"
"<label class='wl'>Active provider<span class='inf' onclick='sInfo(\"cprov\")'>i</span></label>"
...
"</select>"
"<label class='wl'>Anthropic API key<span class='inf' onclick='sInfo(\"ckey\")'>i</span></label>"
...
"<label class='wl'>Google Gemini API key<span class='inf' onclick='sInfo(\"gkey\")'>i</span></label>"
"<input class='wi' type='password' id='stGkey' autocomplete='off' placeholder='(not set)'>"
"<label class='wl'>Gemini model<span class='inf' onclick='sInfo(\"gmodel\")'>i</span></label>"
...
"<label class='wl'>SD retention cap (%)<span class='inf' onclick='sInfo(\"cap\")'>i</span></label>"
"<input class='wi' type='number' min='50' max='95' id='stCap'>"
"<label class='wl'>Stream / capture JPEG quality<span class='inf' onclick='sInfo(\"qual\")'>i</span></label>"
...
"<label class='wl'>Resolution (needs reboot)<span class='inf' onclick='sInfo(\"res\")'>i</span></label>"
...
"<label class='wl'>Brightness (auto-exposure level)<span class='inf' onclick='sInfo(\"ae\")'>i</span></label>"
...
"</select>"
"<label class='wl'>NTP server<span class='inf' onclick='sInfo(\"ntp\")'>i</span></label>"
...
"</div>"
/* Shared settings-info modal: one instance, filled by sInfo() from the SINFO
 * table; click the backdrop or the X to dismiss. */
"<div class='ipop' id='ipop' onclick='if(event.target===this)ipClose()'>"
"<div class='ipbox'><span class='ipx' onclick='ipClose()'>&#10005;</span>"
"<h4 id='ipT'></h4><div id='ipD'></div>"
 ...
 *   YELLOW CLASSIFYING      — iNat scoring the normal frames
 * Motion wins over fastbird wins over classify, so exactly one lamp shows. */
"var db=$g('detbadge'),cb=$g('clsbadge'),fb=$g('fbbadge'),lw=$g('liveWrap');"
"var onFast=!!s.fastBird&&!s.motion;"
"var busyCls=!!s.clsBusy&&!s.motion&&!onFast;"
"if(db)db.classList.toggle('on',!!s.motion);"
"if(fb)fb.classList.toggle('on',onFast);"
"if(cb)cb.classList.toggle('on',busyCls);"
"if(lw){lw.classList.toggle('det',!!s.motion);lw.classList.toggle('fb',onFast);lw.classList.toggle('cls',busyCls);}"
...
"var g_savedRes=1;"
/* Per-setting info popup content: key -> [title, description, default,
 * alternatives]. Rendered via innerHTML (entities allowed). Defaults are the
 * compiled-in settings.c values — keep them in sync when a default changes. */
"var SINFO={"
"mode:['Placement','Records whether the box is mounted as a nest box or watching a feeder."
" Currently informational only &mdash; it does not change detection or identification behaviour.',"
"'Feeder','Nest box'],"
"sens:['Motion sensitivity','How much of the detection zone must change (as one connected blob)"
" to count as motion: 0 needs ~11% of the zone, 50 needs ~6%, 100 only ~1%. Higher = more"
" sensitive. Scattered wind-blown change (grass, foliage) is ignored &mdash; only the largest"
" connected cluster counts, so a compact bird-sized blob wins over diffuse noise.',"
"'50','0&ndash;100. Lower it if leaves or shadows trigger events; raise it if small birds are missed.'],"
"ccnt:['Frames per event','How many photos one motion event captures. More frames give species ID"
" more chances (at least 2 frames must agree on an iNaturalist result) and a better best shot for"
" the Gallery, at the cost of SD space and a longer busy period per event.',"
"'4','1&ndash;10. 1 disables corroboration; 3&ndash;7 is the useful range.'],"
"civl:['Frame interval','Pause between the frames of one event. Short = a near-burst of the same"
" pose; long = the visit sampled over more seconds, with a better chance of a clear pose but some"
" risk the bird leaves mid-event.',"
"'1000 ms','250&ndash;10000 ms in 250 ms steps.'],"
"cool:['Cool-down between events','After an event finishes, ignore motion for this long, so one"
" long visit does not become dozens of events (and dozens of identification calls).',"
"'120 s','1&ndash;600 s. Raise it on a busy feeder to save SD space and API calls; lower it to"
" catch more distinct visits.'],"
"qtn:['Detection quarantine after boot','After a reboot, ignore motion for this many seconds so the"
...
" per frame. Saved photos are unaffected; the detection zone is set on the Live tab.',"
"'Off','On. Note: the current iNat model often scores tight crops <i>worse</i> than the whole"
" frame, so leave this off unless small birds routinely go unidentified.'],"
"fshut:['Reduce motion blur (fast shutter)','When the scene reads dark, fixes the exposure short"
" instead of letting it lengthen &mdash; the main cause of a blurry close or fast-moving bird at"
" dusk or indoors. Only engages in low light (same brightness check as the illuminator), so"
" daylight shots are unaffected; low-light shots come out darker/noisier in exchange for less blur.',"
"'Off','On.'],"
"conf:['Confidence threshold','Minimum confidence for a cloud (Claude/Gemini) identification to be"
" accepted; below it the event stays unidentified rather than risk a wrong label. Also defines the"
" review band the Gallery&rsquo;s &ldquo;Near threshold&rdquo; filter shows.',"
"'30%','15&ndash;95%. Lower admits more (riskier) automatic labels; higher leaves more events for"
" manual review.'],"
...
" per-call fee. Needs the login or a token below, plus WiFi.',"
"'Off (opt-in)','On &mdash; the intended mode; with this off and no cloud provider, new events"
" stay unidentified.'],"
"iuser:['iNaturalist login','With your login stored the box signs in by itself the way a browser"
" does, mints its own session cookie and keeps its ~24 h token fresh indefinitely &mdash; nothing"
" to paste, ever. Press <b>Log in now</b> after saving to test it. iNaturalist has no permanent"
" API key, so this is the recommended setup.',"
"'(not set)','Leave unset and paste the session cookie or a token below instead.'],"
"loc:['Location (nearest capital)','iNaturalist is much more accurate with a location &mdash; it"
" favours species that actually occur near you. Pick the capital nearest the camera. Without it"
" iNat runs vision-only and can suggest birds from the wrong continent at low confidence.',"
"'Oslo, Norway (59.91,10.75)','Any capital in the list, sent as lat,lng with each identification"
" call; or no location (vision-only, weaker results).'],"
"isess:['Session cookie','Manual fallback: paste the <b>_inaturalist_session</b> cookie and the box"
" mints fresh 24 h tokens from it for weeks. Get it after logging in at inaturalist.org: DevTools"
" &rarr; Application &rarr; Cookies &rarr; inaturalist.org &rarr; copy the value. The automatic"
" login produces exactly this for itself. Kept out of the settings export.',"
"'(not set)','Not needed while the stored username/password login works.'],"
"ikey:['iNaturalist API token (JWT)','Manual fallback of last resort: a hand-pasted token from"
" <b>inaturalist.org/users/api_token</b> stops working after ~24 h. The whole"
" <code>{&quot;api_token&quot;:&quot;...&quot;}</code> JSON can be pasted &mdash; the box extracts"
" it. Blank keeps the current token. Kept out of the settings export.',"
"'(not set)','Prefer the stored login or the session cookie &mdash; both outlive it.'],"
"cprov:['Active cloud provider','When iNaturalist cannot identify an event, its best photo goes to"
" this provider and that answer is used instead; if the call fails (no internet, bad key, no"
" credit) the iNaturalist result or <i>Unidentified</i> stands, so nothing is left worse off."
" Each escalated event costs a fraction of a US cent to a couple of cents &mdash; a busy feeder"
" can run to a few dollars a day. Both keys can be stored; this selector picks which one runs,"
" one at a time. Regardless of the selection, the Gallery&rsquo;s &#10024; button asks the cloud"
" about a single photo whenever a key is set.',"
"'Off &mdash; iNaturalist only','Anthropic Claude (Opus 4.8) or Google Gemini (2.5 Flash).'],"
"ckey:['Anthropic API key','Create one at console.anthropic.com/settings/keys. Stored on the device"
" and sent only to api.anthropic.com; never shown again once saved and kept out of the settings"
" export. Blank keeps the current key; the Forget button removes it.',"
"'(not set)','&mdash;'],"
"gkey:['Google Gemini API key','Create one at aistudio.google.com/apikey. Stored on the device and"
" sent only to generativelanguage.googleapis.com; never shown again once saved and kept out of the"
" settings export. Blank keeps the current key; the Forget button removes it.',"
"'(not set)','&mdash;'],"
"gmodel:['Gemini model','Which Gemini model to call. Availability changes with your Google account"
" and as generations retire: if a call 404s, press <b>Test Gemini</b> to list the model ids your"
...
"'80%','50&ndash;95%.'],"
"qual:['Stream / capture JPEG quality','JPEG compression for the live stream and saved photos."
...
" value also appears here.'],"
"ntp:['NTP server','Where the clock syncs from at boot. Until the first sync the clock reads ~1970"
" and captures are quarantined or filed under no-date, so a reachable server matters.',"
"'pool.ntp.org','Regional pools, time.cloudflare.com, time.google.com, time.nist.gov, or a custom"
" server.'],"
