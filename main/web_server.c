...
"<button class='act' style='margin:0;background:#3f8a4f' onclick='gConfirmSel()'>&#10004; Confirm selected</button>"
"<button class='act' style='margin:0;background:#3f6a8a' onclick='gSpecToggle()'>&#128278; Set species&#9662;</button>"
...
"function gSeq(url,fs,bodyFn,cb){var i=0,ok=0;(function nxt(){"
"if(i>=fs.length){cb(ok);return;}"
"$g('gselc').textContent=(i+1)+'/'+fs.length+'\\u2026';"
"fetch(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:bodyFn(fs[i])})"
".then(r=>r.json()).then(function(o){if(o&&o.ok)ok++;}).catch(()=>{}).then(function(){i++;nxt();});})();}"
...
"function(ok){$g('gselc').textContent=ok+' confirmed';loadDay();});}"
...
"+'&c='+encodeURIComponent(m.c)+'&l='+encodeURIComponent(m.l);},"
"function(ok){g_lastSp={c:m.c,l:m.l,d:v};$g('gspecbar').style.display='none';loadDay();});}"
